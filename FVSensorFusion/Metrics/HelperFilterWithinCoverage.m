...
% Copyright 2020-2023 The MathWorks, Inc.
    ...
    methods(Access = protected)
        function setupImpl(obj)
            ...
            % Get scenario variable for driving scenario test bench and
            % actorProfiles for RoadRunner based test bench model.
            if evalin("base","exist('scenario','var')")
                obj.scenario = evalin('base', 'scenario');
            else
                obj.actorProfiles = evalin('base', 'actorProfiles');
            end
        end
        ...
        function [truth] = stepImpl(obj,actors)
            ...
            for i=1:actors.NumActors
                ...
                % Get actor legth, width and rear overhang based on whether
                % the test bench uses driving scenario or RoadRunner.

                if evalin("base","exist('scenario','var')")
                    actorLength = obj.scenario.Actors(i+1).Length;
                    actorWidth = obj.scenario.Actors(i+1).Width;
                    actorRearOverhang = obj.scenario.Actors(i+1).RearOverhang;
                else
                    actorLength = obj.actorProfiles(i+1).Length;
                    actorWidth = obj.actorProfiles(i+1).Width;
                    actorRearOverhang = obj.actorProfiles(i+1).RearOverhang;
                end
                ...
                positionVehicleCenter = driving.scenario.internal.Utilities.translateVehiclePosition(...
                    ...
                    actorRearOverhang,...                % veh.RearOverhang,
                    actorLength,...
                    groundTruth(i).Roll,...
                    groundTruth(i).Pitch,...
                    groundTruth(i).Yaw);
