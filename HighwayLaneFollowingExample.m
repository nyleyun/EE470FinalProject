...
% is used to model the ego vehicle.
...
% 
% To explore the test bench model, load the highway lane following project.
...
% detect the vehicles.
% * The <docid:driving_ref#mw_a657f4e8-85c3-41ea-8d9c-1146805720f0 Simulation 3D Probabilistic Radar> 
% block is attached to the ego vehicle to detect vehicles in 3D simulation 
% environment.
% * The <docid:driving_ref#mw_e07bfe36-17d0-4326-b961-8c7c2aa0fc71 Simulation 3D Vision Detection Generator>
% block generates the actor position truth from the from camera measurements 
% taken by a vision sensor mounted on an ego vehicle in a 3D simulation 
% environment. 
%
...
% Model>.
% 
% The |Metrics Assessment| subsystem enables system-level and component-level 
% metric evaluations using the ground truth information from the scenario. 
% Open the |Metrics Assessment| subsystem.
open_system("HighwayLaneFollowingTestBench/Metrics Assessment");

%% 
% Using this example, you can evaluate the system-level behavior using four
% system-level metrics. Additionally, you can also compute component-level
% metrics to analyze individual components and their impact on the overall
% system performance. 
%
...
%
% * Lane Metrics &mdash; This block verifies that distances between the
% detected lane boundaries and the ground truth data are within the
% thresholds specified in a test scenario.
% *  Vehicle Detector Metrics &mdash; This blocks computes and logs
% true positives, false negatives, and false positives for the detections.
% *  Sensor Fusion & Tracking Metrics &mdash; This subsystem computes
% generalized optimal subpattern assignment (GOSPA) metric, localization
% error, missed target error, and false track error. For more information
% on these metrics, see <docid:driving_ug#mw_b1548d1e-5e35-47b2-9d5f-f556f8926286
% Forward Vehicle Sensor Fusion> example.
%
% You can integrate these metrics with Simulink Test&trade; and enable 
% automatic regression testing. For more information, see 
% <docid:driving_ug#mw_0ca730ea-a0be-4ce1-a53f-2cb78c27a7f8 Automate 
% Testing for Highway Lane Following> example.
...
% 
% The forward vehicle sensor fusion component fuses the vehicle detections
