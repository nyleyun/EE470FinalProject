  ...
  properties (SetAccess='private', GetAccess='private', Hidden)
    ...
    methods(Access = protected)
        function setupImpl(obj)
            ...
            obj.MaxBBoxWidth = ceil(camera.ImageSize(2)*0.878);
            
             % Maximum height of the bounding boxes allowed
            obj.MaxBBoxHeight = ceil(camera.ImageSize(1)*0.78);
            
            % Region of interest
            obj.ROI = [1,1,camera.ImageSize(2),camera.ImageSize(1)*0.625];
            ...
            obj.MinBBoxXValue = round(camera.ImageSize(2)*0.459);
            
            % Starting pixel of ego vehicle hood
            obj.EgoVehicleHood = round(camera.ImageSize(1)*0.612);
        end
