...
%
% helperSLHighwayLaneFollowingSetup("scenarioFcnName","scenario_LFACC_03_Curve_StopnGo","detectorVariantName","YOLOv2")
% 
% Optional inputs
%   detectorVariantName:
...
%    helperSLHighwayLaneFollowingSetup("detectorVariantName","YOLOv2");
%    helperSLHighwayLaneFollowingSetup("detectorVariantName","YOLOv2", "scenarioFcnName","scenario_LFACC_01_Curve_DecelTarget");
%
...
setSim3DSceneDesc(blkSim3DConfig, scenario, nvp.scenarioFcnName)
...
setStaticRangeConfig(blkMetrics, nvp.scenarioFcnName);
...
allActors = scenario.Actors;
dimensions(size(allActors,2)) = struct();
for i = 1:size(allActors,2)
    actor = allActors(i);
...
assignin('base', 'actorDimensions', dimensions);
...
function setSim3DSceneDesc(blkSim3DConfig, scenario, scenarioName)
...
elseif isequal(scenario.RoadCenters, roadCentersValid.roadCentersCurvedRoadSegment)
    ...
    set_param(blkSim3DConfig, 'ProjectFormat', "Default Scenes");
    set_param(blkSim3DConfig, 'SceneDesc', sceneDesc);
elseif contains(scenarioName, "RRHighway")
    % Update sim3D scene config block for Road Runner game.
    pathToFBX = fullfile(pwd,"RRHighway.fbx");
    
    % Set block parameters
    if exist(pathToFBX, 'file')
        projectFormat = get_param(blkSim3DConfig, 'ProjectFormat');
        if projectFormat ~= "RoadRunner"
            set_param(blkSim3DConfig, 'ProjectFormat', "RoadRunner");
        end
        set_param(blkSim3DConfig, 'RoadRunnerProjectPath', pathToFBX);
    end

    % Update weather conditions for better lighting
    set_param(blkSim3DConfig, 'EnableWeather', 1);
    set_param(blkSim3DConfig, 'SunAltitude', "58.59");
    set_param(blkSim3DConfig, 'SunAzimuth', "266.37");
    set_param(blkSim3DConfig, 'clouds', "15");
    set_param(blkSim3DConfig, 'wind', "10");
    set_param(blkSim3DConfig, 'fog', "30");


    % Update maximum number of tracks per cluster
    load_system('ForwardVehicleSensorFusion')
    hws = get_param('ForwardVehicleSensorFusion', 'modelworkspace');
    hws.assignin('numTracksPerCluster', 15);
else
    error("Road centers do not match supported road types.");
end
end

function setStaticRangeConfig(blkMetrics, scenarioName)
...
if contains(scenarioName, "RRHighway")
